...
class ClusterSubmitter(object):
    """A class for handling cluster submission This handles.
    ...
    def __init__(  # type: ignore
        ...
        flush_factor,  # type: ignore # How many commands to bunch into a single job
        job_name,  # type: ignore # A string name for the overall submissions
        run_locally=False,  # type: ignore # Can optionally run locally instead of submitting (many other args can be ignored in this case)
        memory=None,  # type: ignore # Required memory [MB]
        disk_space=None,  # type: ignore # Required disk space [MB]
        wall_time=None,  # type: ignore # Wall time (e.g. when a job will be terminated) [hrs]
        submit_dir=None,  # type: ignore # Where the submission files will be created and run from
        output_dir=None,  # type: ignore # Where the ouput/error files and the job steering file will go
        ...
        job_mode="wrapper",  # type: ignore # Various options for which mode to use for generating the job, see "job.py" for details
        dry_run=False,  # type: ignore # Dry run (will not actually submit)
        cluster_name=None,  # type: ignore # Optionally specify the cluster name manually (otherwise it is autmatically determined, which is recommended)
        # Environment setup
        job_env_vars=None,  # type: ignore # Dict of env vars to set on the nodes
        export_env=False,  # type: ignore # Export submission session env. Not recommended to use this, instead pass sectup script to `start_up_commands`
        env_shell=None,  # type: ignore # Specify an IceCube env-shell.sh script to use
        start_up_commands=None,  # type: ignore # Commands to be run at start of job
        tear_down_commands=None,  # type: ignore # Commands to be run at start of job
        # Node requirements
        require_cvmfs=False,  # type: ignore # Nodes must have CVMFS
        require_avx=False,  # type: ignore # Nodes must have AVX
        require_cuda=False,  # type: ignore # Nodes must have CUDA
        require_sl7=False,  # type: ignore # Nodes must have Scientific Linux 7
        choose_sites=None,  # type: ignore # List of sites to choose
        exclude_sites=None,  # type: ignore # List of sites to exclude
        num_cpus=1,  # type: ignore # Request CPUs
        num_gpus=0,  # type: ignore # Request GPUs
        # Special args for SLURM
        partition=None,  # type: ignore
        mail_type=None,  # type: ignore
        mail_user=None,  # type: ignore
        # Special args for Condor
        accounting_group=None,  # type: ignore
    ):  # type: ignore
        """test."""
        #
        # Store args
    ...
    def _init_cluster_submission(self, cluster_name=None):  # type: ignore
    ...
    def _init_cluster_site(self, cluster_name=None):  # type: ignore
        """test."""
        ...
        self.specify_cluster_name(cluster_name, icecube_clusters)
    ...
    def specify_cluster_name(self, cluster_name, icecube_clusters):  # type: ignore
        """test."""
    ...
    def add(  # type: ignore
        ...
        allowed_return_status=DEFAULT_ALLOWED_RETURN_STATUS,  # type: ignore
    ):  # type: ignore
        """test."""
    ...
    def _submit_to_cluster(self):  # type: ignore
    ...
    def _run_jobs_locally(self):  # type: ignore
    ...
    def clear_footprint(self):  # type: ignore
...
#        disk_space=1000,
#        submit_dir="./tmp",
#        #start_up_commands=["source "+os.path.abspath(os.path.expandvars("$FRIDGE_DIR/setup_fridge.sh"))],
#        start_up_commands=["source "+os.path.abspath(os.path.expandvars("/groups/icecube/qgf305/graphnet_user/run_training_example.sh"))],
#        # start_up_commands=["echo 'I am a start-up command' ; pwd"],
#         tear_down_commands=["echo 'I am a tear-down command'"],
#        run_locally=False,
#    ) as submitter :
#
#        print("Testing submitter")
#
#        submitter.add(command="echo 'Command 0, sleep for 5' ; sleep 5",description="A description",allowed_return_status=[5,0]) # Define acceptable return status values
# submitter.add(command="echo 'Command 1, sleep for 10' ; sleep 5",allowed_return_status=None) # No check on return status
# submitter.add(command="echo 'Command 2, create bar file' ; touch bar")
# submitter.add(command="echo 'Command 3, do some python' ; python -c 'print(\"I am python\")'")
