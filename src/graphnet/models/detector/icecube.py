...
class IceCubeUpgrade_V2(IceCubeDeepCore):
    ...
    def _forward(self, data: Data) -> Data:
        ...
        pmt_dir = torch.stack((data['pmt_dir_x'], data['pmt_dir_y'], data['pmt_dir_z']), dim=1)
...
class IceCubeUpgrade_V3(IceCubeDeepCore):
    """`Detector` class for IceCube-Upgrade Noise classification."""
    ...
    def _forward(self, data: Data) -> Data:
        ...
        # exclude specific dom types from data
        dom = {'pdom':20, 'WOM':110, 'Degg':120, 'mdom':130}
        #data = data.loc[data['dom_type'] != dom['pdom']]
        data = data.loc[data['dom_type'] != dom['WOM']]
        data = data.loc[data['dom_type'] != dom['Degg']]
        data = data.loc[data['dom_type'] != dom['mdom']]
        ...
        #data['center_of_gravity'] = data.x[:,15]
